...
import swen222.concurrent.Job;
import swen222.concurrent.ThreadPool;
...
public class ParSort {
	...
	public static void parallelSort(List<Integer> data, int numProcessors) {
		...
		int	regSize = data.size() / numProcessors;

		int endSize = regSize;
		int count = 0;
		for (int i = 0; i < (numProcessors - 1); i++) {

			jobs[i] = new SortJob(data, count, endSize);
			pool.submit(jobs[i]);
			// job.waitUntilFinished();
			count = count + regSize;
			endSize = endSize + regSize;

		}
		int lastIndex = jobs.length - 1;
		jobs[lastIndex] = new SortJob(data, count, data.size());
		pool.submit(jobs[lastIndex]);
		
		for(int i=0; i<numProcessors; i++){

			SortJob temp = jobs[i];
			temp.waitUntilFinished();
